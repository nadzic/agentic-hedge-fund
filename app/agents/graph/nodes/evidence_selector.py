...
_USED_FOR_MARKERS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ("what_changed", ("yoy", "qoq", "increased", "decreased", "declined", "rose", "fell", "guidance")),
    ("what_matters_now", ("demand", "margin", "guidance", "risk", "cash flow", "backlog", "headwind")),
    ("bull_points", ("beat", "strong", "growth", "expansion", "improved", "accelerat")),
    ("bear_points", ("miss", "weak", "decline", "pressure", "slowdown", "risk", "headwind")),
    ("what_to_watch_next", ("next quarter", "outlook", "watch", "monitor", "going forward", "expects")),
)
...
def _should_exclude(ev: dict[str, Any], text: str) -> str | None:
    ...
    if strength == "weak" and fact_or_interpretation == "interpretation" and _contains_any(lower, _SPECULATION_MARKERS):
        return "Weak speculative interpretation without supporting reporting."
    if source_type in {"analyst_or_commentary", "public:analyst_or_commentary"} and not has_numeric and strength == "weak":
...
def _inclusion_score(ev: dict[str, Any], reporting_relevance_score: float) -> float:
    ...
    try:
        final_source_score = float(final_source_score_raw) if final_source_score_raw is not None else 0.5
    except Exception:
  ...
  "decisions": [
    {
      ...
      "reason": "string",
      "inclusion_score": 0.0,
      "used_for": ["what_changed", "what_matters_now", "bull_points", "bear_points", "what_to_watch_next"]
    }
  ]
}
""".strip(),
    ...
    for item in raw_decisions:
        ...
        try:
            inclusion_score = float(item.get("inclusion_score"))
        except Exception:
            inclusion_score = 0.0
        out[i] = {
    ...
    try:
        ...
        for idx, raw in enumerate(classified):
            ...
            if llm_item is not None:
                include = bool(llm_item.get("include"))
                ev["include"] = include
                ev["reason"] = str(llm_item.get("reason") or "").strip() or (
                    "Included by prompt-driven selection." if include else "Excluded by prompt-driven selection."
                )
                ev["inclusion_score"] = float(llm_item.get("inclusion_score") or 0.0)
            ...
            inclusion_score = _inclusion_score(ev, reporting_score)
            ...
            if strength == "weak":
                # Weak evidence can be included only if it helps explain market debate and is clearly weak.
                include = (
                    inclusion_score >= 0.58
                    and fact_or_interpretation == "interpretation"
                    and any(tag in used_for for tag in ("bull_points", "bear_points", "what_matters_now"))
                )
            ...
            ev["include"] = include
            if include:
                ev["reason"] = f"Included: relevant to latest reporting with inclusion_score={inclusion_score:.2f}."
                selected.append(ev)
            else:
                ev["reason"] = (
                    f"Excluded: below selection threshold (score={inclusion_score:.2f}) or low evidence utility."
                )
                discarded.append(ev)
        ...
        return {
            ...
            "warning": " | ".join(
                part
                for part in [str(state.get("warning") or "").strip(), llm_warning]
                if part
            )
            or None,
            "error": None,

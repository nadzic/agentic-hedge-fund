...
def valuation_analyst_node(state: WorkerState) -> dict[str, list[AnalystOutput]]:
    ...
    try:
        decision, metrics = run_valuation_analysis(symbol=symbol, horizon=horizon)
        fallback_reasoning = (
            f"Valuation for {symbol} ({horizon}): signal={decision.signal.value}, "
        ...
        return {
            "analyst_outputs": [
                AnalystOutput(
                    ...
                    signal=Signal.HOLD,
                    confidence=0.50,
                    reasoning=f"Valuation fallback due to error: {exc}",
                    metrics={
                        "valuation_score": 0.0,
                        "status": "fallback",

...
_METRIC_KEYWORDS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ...
    ("management_commentary", ("management said", "management noted", "ceo said", "cfo said", "prepared remarks")),
...
def _detect_comparison_type(text: str) -> str | None:
    lower = text.lower()
    if any(marker in lower for marker in ("yoy", "year over year", "vs last year", "from last year")):
        return "YoY"
    if any(marker in lower for marker in ("qoq", "quarter over quarter", "sequential", "vs previous quarter")):
        return "QoQ"
    if any(marker in lower for marker in ("guidance", "outlook", "forecast", "expects", "expected")):
        return "guidance_change"
    if any(marker in lower for marker in ("management said", "management noted", "commented", "highlighted")):
...
def _extract_evidence_from_chunk(chunk: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    source = str(chunk.get("source_id") or chunk.get("url") or "").strip() or None
    rank = chunk.get("rank")
    try:
  ...
  "evidences": [
    {
      "source_idx": 0,
      "claim": "string",
      "category": "financial_result|margin|guidance|segment_performance|management_commentary|demand_signal|cost_capex_signal|risk_disclosure|market_reaction|general",
      "period": "string|null",
      "metric": "string|null",
      "value": "string|null",
...
""".strip(),
    ...
    for idx, ev in enumerate(raw_evidences, start=1):
        ...
        comparison_type = str(ev.get("comparison_type") or "").strip() or _detect_comparison_type(claim)
        out.append(
            {
                "claim": claim,
                "category": category,
...
def evidence_extractor_node(state: Mapping[str, object]) -> dict[str, object | None]:
    ...
    try:
        ...
        try:
            ...
            if llm_evidences:
                return {"evidences": llm_evidences[:80], "warning": state.get("warning"), "error": None}
